...
void slightRight(){
    goRightMotor(98);  // previously is 120, but keep refusing to move or stuck, so increase to 130
    // delay(5);
    // restMotor();
}

void slightLeft(){
    goLeftMotor(98); // previously is 120, but keep refusing to move or stuck, so increase to 130
    // delay(5);
    // restMotor();
}